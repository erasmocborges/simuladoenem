...
export function StudentAuthDialog({ open, onOpenChange, initialMode = "login", context = "student", onLogin, onSignup, onRecover, onGoogleLogin, requiresPasswordReset, onCompletePasswordRecovery }: StudentAuthDialogProps) {  const [mode, setMode] = useState<AuthMode>("login");
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error("Falha na autenticação", { status: summary.status, code: summary.code, message: summary.message });
  ...
  const description = mode === "login"
    ? isDeveloperAccess
      ? `Use a conta docente autorizada. O acesso exige ${AUTHORIZED_TEACHER_EMAIL} e o papel teacher atribuído no Supabase.`
      : "Use sua conta vinculada ao Supabase para sincronizar seu progresso."
    : mode === "signup"
